...
export default function EditarPresbitero() {
  ...
  const [formData, setFormData] = useState<{
    ...
    ocupacao_canonica: string
    nome_paroquia: string
    foto_perfil: File | null | string
    documento_ordenacao: File | null | string
    facebook: string
    instagram: string
    website: string
    email: string
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const fieldName = e.target.name as "foto_perfil" | "documento_ordenacao"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let foto_perfil_path = formData.foto_perfil
      let documento_ordenacao_path = formData.documento_ordenacao

      if (formData.foto_perfil instanceof File) {
        foto_perfil_path = await uploadFile(formData.foto_perfil, "fotos_perfil")
      }

      if (formData.documento_ordenacao instanceof File) {
        documento_ordenacao_path = await uploadFile(formData.documento_ordenacao, "documentos_ordenacao")
      }
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          documento_ordenacao: documento_ordenacao_path,
        })
        .eq("id", id)
